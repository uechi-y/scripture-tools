        ...
        Array(chaptersCount)
            ...
            .map(async (_, index) => {
                ...
                const rawVerses = await collect(
                    `https://www.churchofjesuschrist.org/study/scriptures/${volume}/${book}/${chapter}?lang=kor`
                );
...
async function collectVolume(volume: VolumeDescription): Promise<Volume> {
    const { name, books: bookNames } = volume;

    const books = await Promise.all(bookNames.map(book => collectBook(name, book)));
    return {
        name,
        books,
    };
...
const volume = await collectVolume(BOOK_OF_MORMON);
